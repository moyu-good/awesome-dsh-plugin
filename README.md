...
- [moyu-good/dsh-lark-bridge](https://github.com/moyu-good/dsh-lark-bridge) — Feishu/Lark IM channel for DeepSeek Harness: native reasoning (CoT) messages, approval cards that record the decider, live goal/todo cards, subagent fan-out, context-compression transparency, in-chat scheduled reminders, per-chat/thread/sender agent scoping, WebSocket long-connection (no public callback URL), QR signup, zero-build install.
